...
export const FlowCreate: React.FC = () => {
    const { formProps, saveButtonProps } = useForm({
        onMutationSuccess: () => {
            // Handle success - maybe redirect to flows list or show success message
    ...
    const handleCreateFlow = async () => {
        try {
            ...
            // Get form values
            const formValues = formProps.form?.getFieldsValue() as {
                name: string;
                description?: string;
                bucket_id: string;
            };
            ...
            const flowData = {
                ...
                bucket_id: formValues.bucket_id,
            ...
            message.success(`Flow "${formValues.name}" created successfully!`);
            
            // Optionally redirect to the flows list or the created flow
    ...
    const renderFlowForm = () => (
        <Form {...formProps} layout="vertical" onFinish={() => setCurrentStep(2)}>
            <Form.Item
                label="Flow Name"
                name="name"
                rules={[{ required: true, message: 'Please enter flow name' }]}
                initialValue={selectedTemplate?.name ? `${selectedTemplate.name} Flow` : ''}
            >
            ...
            <Form.Item name="template_id" style={{ display: 'none' }}>
                <Input value={selectedTemplate?.id} />
            </Form.Item>
