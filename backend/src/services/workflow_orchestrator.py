...
class WorkflowOrchestrator(LoggerMixin):
    ...
    async def import_and_deploy_flow(
        ...
        """
        Import a flow from Registry and deploy to NiFi with parameters.
        """
        parameters = parameters or {}

        try:
            self.logger.info("Starting import and deploy workflow for Registry flow: %s", flow_id)

            # Step 1: Import flow from Registry to NiFi
            import_result = await self.integration_bridge.import_flow_from_registry(
                bucket_id=bucket_id,
                flow_id=flow_id,
                version=version,
                parent_group_id=parent_group_id,
            ...
            if not import_result.get("success"):
                return {
                    "workflow": "import_and_deploy",
                    "bucket_id": bucket_id,
                    "flow_id": flow_id,
                    "success": False,
                    "stage": "registry_import",
                    "import_result": import_result,
                    "message": "Flow import from Registry failed"
                }

            process_group_id = import_result.get("process_group_id")

            # Step 2: Apply parameters if provided
            if parameters:
                param_result = await self.nifi_param_mgmt.create_parameter_context(
                    name=f"params-{flow_name or flow_id}",
                    description=f"Parameters for imported flow {flow_id}",
                    parameters=parameters
                )
                ...
                if param_result.get("success"):
                    await self.nifi_param_mgmt.assign_parameter_context_to_process_group(
                        process_group_id=process_group_id,
                        parameter_context_id=param_result.get("parameter_context_id")
                    )

            # Step 3: Get final flow status
            ...
            result = {
                ...
                "bucket_id": bucket_id,
                "flow_id": flow_id,
                "imported_version": import_result.get("imported_version"),
                "success": True,
                "stage": "completed",
                "import_result": import_result,
                "parameters_applied": len(parameters) > 0,
                "flow_status": flow_status,
                "process_group_id": process_group_id,
                "message": "Flow successfully imported from Registry and deployed to NiFi"
            }

            self.logger.info("Successfully completed import and deploy workflow for flow: %s", flow_id)
            return result

        except Exception as exc:
            self.logger.error("Import and deploy workflow failed for flow %s: %s", flow_id, exc)
            raise WorkflowOrchestratorError(f"Import and deploy workflow failed: {exc}") from exc
