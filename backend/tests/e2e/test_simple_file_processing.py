...
def _determine_test_data_root() -> Path:
    ...
    if _is_codex_environment():
        ...
        nifi_root = Path("/tmp/nifi-working/e2e_test_files")
        try:
        ...
        repo_root = Path(__file__).parent.parent.parent.parent / "tmp" / "nifi-working" / "e2e_test_files"
        try:
...
async def test_simple_file_processing_flow(orchestrator, nifi_client, registry_client):
    ...
    nifi_dirs = test_dirs.to_nifi_paths()
    parameters = {
        "input_directory": str(nifi_dirs.input),
        "output_directory": str(nifi_dirs.output),
        "input_pattern": ".*\\.txt$",
    }
