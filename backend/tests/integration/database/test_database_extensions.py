...
async def test_pgvector_extension_is_active(db_session):
    ...
    try:
        ...
        # Consume the result completely to ensure query finishes
        rows = result.fetchall()
        assert len(rows) > 0, "Query should return a result"
        # No need to manually commit - let fixture handle it
...
async def test_apache_age_extension_is_active(db_session):
    ...
    try:
        ...
        result = await db_session.execute(text("SELECT * from cypher('test_graph', $$ MATCH (n) RETURN n $$) as (v agtype);"))
        # Consume the result completely to ensure query finishes
        rows = result.fetchall()
