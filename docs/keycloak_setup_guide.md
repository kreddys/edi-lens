...
-   `partner:create`: Can create new trading partners.
-   `partner:read`: Can view trading partner configurations.
-   `partner:update`: Can update existing trading partners.
-   `partner:delete`: Can delete trading partners.
...
-   **`tenant-admin`**: Has full control over a single tenant's resources (all `partner:*`, `user:*`, `billing:*`, and `validation:run` permissions).
-   **`tenant-editor`**: Can manage trading partners but not users or billing.
-   **`tenant-viewer`**: Has read-only access (`partner:read`, `validation:run`).
