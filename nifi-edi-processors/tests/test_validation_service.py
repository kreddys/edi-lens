...
class TestEDIValidationService:
    ...
    def test_validation_with_empty_content(self):
        ...
        with patch.object(self.validation_service.schema_manager, 'get_schema', return_value=MagicMock()):
            result = self.validation_service.validate_edi(
                edi_content="",
...
IEA*1*000000001~"""
    ...
    def test_validation_exception_handling(self):
        ...
        with patch.object(self.validation_service.schema_manager, 'get_schema', side_effect=Exception("Schema loading failed")):
            result = self.validation_service.validate_edi(
                edi_content=edi_content,
                schema_name="837p.json",
                tenant_id="test-tenant",
